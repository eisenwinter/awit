...
brief: >-
  Replace the detectCycles seam with Tarjan SCC over Node.Deps. Every SCC of size greater than 1, or a size-1 self-edge, quarantines every member with ReasonCycle, records one graph-level Fault, and prints a deterministic example chain plus a dep-rm fix.
status: closed
deps: [AWIT-0ND56N3G]
labels: [phase2, p0]
refs_base: repo
