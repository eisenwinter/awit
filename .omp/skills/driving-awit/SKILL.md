...
- **Put debug evidence in `comment --file`, never new `.awit/` paths.** For JSON dumps, CSVs, or logs, `awit comment <id> --file debug.json` copies the file into the item's comments and links it as a ref; explain what it shows in a text comment. Never manually create `.awit/evidence/` or any new file/directory under `.awit/`: unknown paths lack a lifecycle (archive/quarantine ignore them) and rot.
- **Only `next --claim` commits**, exposing double claims as merge conflicts and quarantine. Policy: `--commit=true|false` overrides the repository default `commit:` in `config.yaml` (omitted means true); `--no-commit` is deprecated `--commit=false`. `comment`, `close`, `release`, `create`, `update`, `dep`, `ref`, `skill sync` leave changes uncommitted. Commit `.awit/` with your code: `awit: close <id>` after close, `awit: release <id>` after release, or others still see your claim. Check `.omp/agents/`, especially `.omp/agents/orchestrator.md`: if the orchestrator owns commits, report instead of committing.
...
| Refresh agent skills        | `awit skill sync` (detected dirs only; `current`/`updated`/`created` plus path; overwrites hand-edits; inherited `--repo`; exit 0 even with none detected)                                                                                                                                                                                        |
