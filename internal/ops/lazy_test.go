...
func newLazyOps(t *testing.T, fixture string) (*ops.Lazy, *item.Store, string) {
	t.Helper()
	dir := copyFixture(t, fixture)
	s := openTestStore(t, dir)
	return ops.NewLazy(s, "claude", fixedNow), s, dir
}
...
func TestLazyOpsBytesMatchCLI(t *testing.T) {
	type op struct {
		name string
		id   string
		tui  func(o *ops.Lazy) error
		cli  []string
	}
	cases := []op{
		{"close", "AWIT-TEST0001", func(o *ops.Lazy) error { return o.Close("AWIT-TEST0001", "") }, []string{"close", "AWIT-TEST0001", "--no-push"}},
		{"block", "AWIT-TEST0002", func(o *ops.Lazy) error { return o.Block("AWIT-TEST0002", "vendor") }, []string{"block", "AWIT-TEST0002", "--reason", "vendor"}},
		{"release", "AWIT-TEST0006", func(o *ops.Lazy) error { return o.Release("AWIT-TEST0006") }, []string{"release", "AWIT-TEST0006", "--no-push"}},
		{"claim", "AWIT-TEST0002", func(o *ops.Lazy) error { return o.Claim("AWIT-TEST0002") }, []string{"next", "AWIT-TEST0002", "--claim", "--agent", "claude", "--commit=false"}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			got, _ := os.ReadFile(s.ItemPath(c.id))
	...
	t.Run("unblock", func(t *testing.T) {
		...
		before, _ := os.ReadFile(s.ItemPath("AWIT-TEST0002"))
		if err := o.Unblock("AWIT-TEST0002"); err != nil {
			t.Fatal(err)
		}
		after, _ := os.ReadFile(s.ItemPath("AWIT-TEST0002"))
