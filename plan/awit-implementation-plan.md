...
Twenty commands; `-p` is gone everywhere, `release`, `validate`, `label`, `archive`, `import`, `ref`, `external`, `block` and `unblock` are new, and every list-shaped output honours `--format compact|table|json` (compact when stdout is not a TTY).
